...
// InitializePostgreSQLConn :nodoc:
func InitializePostgreSQLConn() {
	conn, err := openPostgreSQLConn(config.DatabaseDSN())
	if err != nil {
	...
	go checkConnection(time.NewTicker(config.PostgreSQLPingInterval()))
...
func openPostgreSQLConn(dsn string) (*gorm.DB, error) {
	...
	conn.SetMaxIdleConns(config.PostgreSQLMaxIdleConns())
	conn.SetMaxOpenConns(config.PostgreSQLMaxOpenConns())
	conn.SetConnMaxLifetime(config.PostgreSQLConnMaxLifetime())
	...
	for {
		...
		case <-ticker.C:
			if _, err := PostgreSQL.DB(); err != nil {
				reconnectPostgreSQLConn()
			}
		}
	}
}

func reconnectPostgreSQLConn() {
	b := backoff.Backoff{
		Factor: 2,
		Jitter: true,
